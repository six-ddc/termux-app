package com.termux.autotermux.input
...
class AutoTermuxKeyboardIME : InputMethodService() {
    private val TAG = "AutoTermuxKeyboardIME"

    companion object {
        private var instance: AutoTermuxKeyboardIME? = null
        
        fun getInstance(): AutoTermuxKeyboardIME? = instance
        
        /**
         * Check if the IME service instance is currently running.
         *
         * This is only true once Android has started this input method service, which usually
         * requires it to be selected. Use isEnabled() for the settings-level enabled state.
         */
        fun isAvailable(): Boolean = instance != null

        /**
         * Check if this IME is enabled in Android input method settings.
         */
        fun isEnabled(context: Context): Boolean {
            val myId = inputMethodId(context)
            val imm = context.getSystemService(Context.INPUT_METHOD_SERVICE) as? InputMethodManager
            if (imm?.enabledInputMethodList?.any { inputMethodInfo ->
                    inputMethodInfo.id == myId ||
                        ComponentName(
                            inputMethodInfo.serviceInfo.packageName,
                            inputMethodInfo.serviceInfo.name,
                        ).flattenToShortString() == myId
                } == true) {
                return true
            }
            ...
            return enabledInputMethods.split(':')
        ...
        private fun inputMethodId(context: Context): String =
            ComponentName(context, AutoTermuxKeyboardIME::class.java).flattenToShortString()
    }

    override fun onCreate() {
        super.onCreate()
        instance = this
        Log.d(TAG, "AutoTermuxKeyboardIME: onCreate() called")
    }
    ...
    override fun onDestroy() {
        Log.d(TAG, "AutoTermuxKeyboardIME: onDestroy() called")
        instance = null
        super.onDestroy()
    }
