...
object TriggerEditorSupport {
    ...
    fun visibilityFor(source: TriggerSource): Visibility {
        return when (source) {
            ...
            TriggerSource.ACTIVITY_CHANGED,
            -> Visibility(
                showMatchMode = true,
                showPackageName = true,
            )
    ...
    fun defaultCooldownSecondsFor(source: TriggerSource): Int {
        return when (source) {
            ...
            -> 30
            else -> if (capabilitiesFor(source).supportsCooldown) DEFAULT_COOLDOWN_SECONDS else 0
        }
    }
    ...
    fun sanitize(rule: TriggerRule): TriggerRule {
        ...
        return rule.copy(
            ...
            notificationDebounceMs = if (isNotificationSource(rule.source)) {
                rule.notificationDebounceMs.coerceAtLeast(0L)
            } else {
                0L
            },
        )
    ...
    fun isNotificationAccessEnabled(context: Context): Boolean {
        val componentName = ComponentName(context, NotificationAccessService::class.java)
