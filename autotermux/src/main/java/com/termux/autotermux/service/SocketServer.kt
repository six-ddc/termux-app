...
class SocketServer(
    ...
    companion object {
        private const val TAG = "AutoTermuxSocketServer"
    ...
    private fun handlePostRequest(
        ...
        try {
            ...
            val params = parsePostParams(postData.toString())
    ...
    private fun parsePostParams(data: String): JSONObject {
        ...
        data.split("&").forEach { pair ->
            ...
            if (parts.size == 2) {
                ...
                val intValue = value.toIntOrNull()
                when {
                    intValue != null -> params.put(key, intValue)
                    value.equals("true", ignoreCase = true) -> params.put(key, true)
                    value.equals("false", ignoreCase = true) -> params.put(key, false)
