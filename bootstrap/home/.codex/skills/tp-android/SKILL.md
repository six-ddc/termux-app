---
name: tp-android
description: Control the Android device from inside Termux through the TermuxPlus `tp-android` CLI. Use whenever an agent needs to take a screenshot, inspect the current UI (tree/find/focus/state), tap/swipe/type/key, launch or stop apps, read or write clipboard, capture audio or screen recordings, manage shared-storage files, install APKs, send notifications, read SMS/call-log/contacts, react to device events, persist trigger rules that auto-run Termux commands, or call any Termux:API-compatible device API. Reach for this skill even when the user just says "tap that button", "take a screenshot", "find the Continue button", "send myself an SMS when X happens", "wait until WhatsApp shows N", or anything that needs to inspect or drive the phone — `tp-android` is the stable surface for all of it.
---
